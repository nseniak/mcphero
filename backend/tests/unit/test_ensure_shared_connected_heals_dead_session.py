...
def _record_reopens(mgr: UpstreamClientManager) -> list[str]:
    """Replace ``_open_shared`` (the reopen body every shared connect
    runs) with a recorder so the heal path can be observed without a real
    MCP handshake."""
    calls: list[str] = []
    ...
    mgr._open_shared = _record  # type: ignore[method-assign]
    return calls
...
async def test_ensure_shared_connected_reconnects_a_dead_session() -> None:
    ...
    assert calls == ["everything2"], (
        ...
        "not reused as a zombie"
    )


@pytest.mark.asyncio
async def test_ensure_and_reconnect_fresh_race_to_one_connect() -> None:
    """R1 cross-pool (review item 3): a heal mid-reopen, racing a concurrent
    lazy ``ensure_shared_connected``, must yield exactly ONE reopen: the
    ensure joins the heal's reopen instead of opening a second sandbox
    (two sandboxes, one orphaned, + a ref race)."""
    upstream = make_upstream_definition(id="everything2")
    mgr = UpstreamClientManager(upstreams=[upstream])
    ...
    async def gated_connect(up: UpstreamDefinition, *_a: Any, **_k: Any) -> Any:
        nonlocal calls
        calls += 1
        await gate.wait()
        # Install a live shared session, as the real reopen would.
        session = cast(Any, object())
        mgr.transition_to_live_shared(
            up.id,
            session=session,
    ...
    mgr._open_shared = gated_connect  # type: ignore[method-assign]

    # Heal: starts the reopen, enters gated_connect, blocks on the gate.
    heal = asyncio.create_task(mgr.reconnect_shared_fresh(upstream))
    await asyncio.sleep(0.05)
    assert calls == 1

    # Concurrent lazy acquire arrives while the heal is reopening.
    ensure = asyncio.create_task(mgr.ensure_shared_connected(upstream))
    await asyncio.sleep(0.05)
    assert calls == 1, "the ensure must join the reopen, not connect in parallel"

    gate.set()  # heal finishes and installs the live session
    await asyncio.gather(heal, ensure)

    assert calls == 1, (
        "ensure must use the heal's fresh session, not open a second "
        "sandbox"
    )
