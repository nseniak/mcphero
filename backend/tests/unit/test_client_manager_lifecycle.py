...
session to another, or ``disconnect_upstream`` leaves a session
dangling, everything downstream lies about connection state.

Tests drive the public state-machine surface (``transition_to_*``)
to seed sessions; assertions read through the typed accessors
(``is_connected``, ``has_user_session``, etc.) and ``get_state``.
...
def _seed_user(
    ...
    """Seed a per-user session: the per-user dicts are populated
    directly (orthogonal to the upstream-level state machine)."""
    session = _stub_session()
    task = _stub_task()
...
# disconnects. It must close the shared session AND leave users'
# sessions alone. If either invariant slips, you either leak sessions
# across restarts or wipe an unrelated user's connection.
...
async def test_disconnect_upstream_does_not_touch_other_upstreams() -> None:
    ...
    slack_alice_task = _seed_user(mgr, "slack", "alice@example.com")

    await mgr.disconnect_upstream("notion")

    notion_task.close.assert_awaited_once()
    slack_task.close.assert_not_awaited()
    slack_alice_task.close.assert_not_awaited()
    assert mgr.is_connected("slack") is True
...
async def test_ensure_shared_connected_noop_when_session_present() -> None:
    ...
    mgr._open_shared = AsyncMock()  # type: ignore[method-assign]

    await mgr.ensure_shared_connected(upstream)

    mgr._open_shared.assert_not_awaited()
...
async def test_ensure_shared_connected_invokes_connect_when_no_session() -> None:
    ...
    opens the shared session exactly once."""
    from tests.unit.factories import make_upstream_definition
    ...
    mgr._open_shared.assert_awaited_once()
...
async def test_ensure_shared_connected_replaces_deferred_with_live_session() -> None:
    ...
    mgr._open_shared = _fake_connect  # type: ignore[method-assign,assignment]
...
async def test_ensure_shared_connected_single_flight() -> None:
    ...
    — the second caller joins the first's connect. Parallel tool calls on
    the same upstream would otherwise stampede E2B with N wakes.
    """
    ...
    async def _slow_connect(*_args: Any, **_kwargs: Any) -> Any:
        nonlocal call_count
        call_count += 1
        started.set()
        await proceed.wait()
    ...
    mgr._open_shared = _slow_connect  # type: ignore[method-assign,assignment]
    ...
    assert call_count == 1, (
        f"the shared session opened {call_count}× — concurrent "
...
# ── Concurrent replace_user_session for same (user, upstream) ──
#
# Two deliberate replacements (two fresh sign-ins) for one user must not
# race their close-then-open sequences. The losing caller's session
# would be leaked: overwritten in ``_user_sessions[key]`` /
# ``_user_tasks[key]`` without anyone awaiting its ``task.close()``.
# Replace semantics ("the latest connect wins, prior session torn down
# cleanly") demand that the second replacement closes the first one's
# task. (Callers that merely need a session do NOT replace; they join
# the connect in flight, see test_user_session_race.py.)


@pytest.mark.asyncio
async def test_concurrent_replace_user_session_does_not_leak() -> None:
    """Concurrent replacements for the same ``(user, upstream)`` must
    serialize: the second one closes the first one's task. Without
    serialization the first task is silently leaked.
    """
    from tests.unit.factories import make_upstream_definition
    ...
    first = asyncio.create_task(
        mgr.replace_user_session(upstream, user)
    )
